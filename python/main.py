...
def main(args):
    if args.use_benchmark:
        if args.make_csv:
            with open('statistic_no_naive.csv', 'w') as file:
        ...
        statistics = pd.read_csv('statistic_no_naive.csv')
        good_statistic = statistics[8:]
        bad_statistic = statistics[:8]
        draw_statistic(good_statistic, 'good_no_naive.png')
        draw_statistic(bad_statistic, 'bad_no_naive.png')
    else:
        ...
        for name, algorithm in algorithms:
            results = algorithm.search()
            print(f"Алгоритм: {name}\t Время: {results.time}\t Ответ: {results.matches}")
